...
const BANNER_RE = /\/\/\s*[─=\-*#]{3,}/;
const MYGRACE_MARKUP_RE =
  /^\s*\/\/\s*===\s*(?:(?:END_)?(?:MODULE_CONTRACT|MODULE_MAP|CHANGE_SUMMARY)|(?:START|END)_(?:CONTRACT(?::\s*[\w$.-]+)?|BLOCK_[A-Z0-9_]+))\s*===\s*$/;
const INCIDENT_RE =
...
describe("comment policy (CLAUDE.md)", () => {
  ...
  test("no banner separator comments", () => {
    const offenders = scan(FILES, (line) => BANNER_RE.test(line) && !MYGRACE_MARKUP_RE.test(line));
