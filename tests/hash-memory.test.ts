...
describe("appendProjectMemory", () => {
  ...
  it("creates MAGRA.md with a header and the first bullet when absent", () => {
    const path = join(dir, "MAGRA.md");
    expect(existsSync(path)).toBe(false);
    const result = appendProjectMemory(dir, "always use pnpm");
    expect(result.created).toBe(true);
    expect(result.path).toBe(path);
    const content = readFileSync(path, "utf8");
    expect(content).toContain("# MAGRA project memory");
    expect(content).toMatch(/- always use pnpm\n$/);
  });

  it("appends to an existing MAGRA.md without disturbing earlier content", () => {
    const path = join(dir, "MAGRA.md");
  ...
  it("appends multiple bullets in order across calls", () => {
    ...
    const content = readFileSync(join(dir, "MAGRA.md"), "utf8");
  ...
  it("respects nested rootDir paths (creates MAGRA.md in the given dir, not cwd)", () => {
    const nested = join(dir, "subproject");
    mkdirSync(nested);
    const result = appendProjectMemory(nested, "scoped note");
    expect(result.path).toBe(join(nested, "MAGRA.md"));
    expect(existsSync(join(dir, "MAGRA.md"))).toBe(false);
  });
});
...
describe("appendGlobalMemory", () => {
  ...
  it("creates ~/.reasonix/MAGRA.md (with parent dir) when missing", () => {
    ...
    expect(result.path).toBe(path);
    const content = readFileSync(path, "utf8");
    expect(content).toContain("# MAGRA global memory");
    expect(content).toMatch(/- always use pnpm\n$/);
  });
  ...
  it("uses os.homedir() when no override is passed (smoke check)", () => {
    ...
    expect(path).toMatch(/[/\\]\.reasonix[/\\]MAGRA\.md$/);
  });
