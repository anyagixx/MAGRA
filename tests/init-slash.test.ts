...
describe("/init slash handler", () => {
  ...
  it("emits the structured init prompt as resubmit when MAGRA.md does not exist", () => {
    const loop = makeLoop();
    const result = handleSlash("init", [], loop, { codeRoot: tmp });
    expect(result.resubmit).toBeDefined();
    expect(result.resubmit).toMatch(/Initialize MAGRA.md/);
  ...
  it("refuses overwriting an existing MAGRA.md without `force`", () => {
    writeFileSync(join(tmp, "MAGRA.md"), "# pre-existing");
    const loop = makeLoop();
    const result = handleSlash("init", [], loop, { codeRoot: tmp });
    expect(result.resubmit).toBeUndefined();
  ...
  it("`/init force` proceeds even when MAGRA.md exists", () => {
    writeFileSync(join(tmp, "MAGRA.md"), "# pre-existing");
    const loop = makeLoop();
