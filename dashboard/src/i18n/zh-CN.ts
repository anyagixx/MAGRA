export const zhCN = {
  ...
  overview: {
    ...
    standaloneDesc:
      "只读磁盘视图。在 magra code 内启动 /dashboard 以获取实时会话状态、MCP 和工具。",
    cockpit: "驾驶舱",
    balance: "余额",
    tokens7d: "tokens · 7 天",
    cacheHit: "缓存命中",
    ...
    budget: "预算",
    currentSession: "当前会话",
    noSession: "无活跃会话 — 在 magra code 内执行 /dashboard 进行连接。",
    promptTok: "提示 tokens",
  ...
  usage: {
    ...
    noData: "暂无用量数据 — 在 magra chat / code / run 中执行一轮，然后刷新。",
    windows: "滚动窗口",
    colWindow: "时间范围",
    colTurns: "轮次",
    colCacheHit: "缓存命中",
  ...
  sessions: {
    ...
    attachRequired:
      "实时会话操作需要已连接的 CLI 会话。请通过 magra chat 启动，或在 TUI 会话中打开仪表盘。",
    activeChip: "当前",
    activePill: "当前",
  },
  tools: {
  ...
  permissions: {
    ...
    standaloneWarning:
      "修改操作需要在活跃的 magra code 会话内执行 /dashboard — 独立模式的 magra dashboard 无法确定要编辑哪个项目的允许列表。",
  },
  mcp: {
    loading: "加载 MCP…",
    ...
    saved: "已保存",
    savedRestart: "已保存 — 重启 magra code 以桥接此服务器",
    ...
    whyUnbridged: "为什么未桥接？",
    whyUnbridgedDesc:
      "此规格存在于您的 config.json 中，但未桥接到实时会话。MCP 服务器在 magra code 启动时连接；仪表盘本身无法生成子进程。",
    whyUnbridgedHint: "激活方法：重启 magra code，然后刷新此仪表盘。",
    bridgeFailed: "桥接失败",
    bridgeFailedTitle: "桥接失败 · 在配置中",
    bridgeFailedHint:
      "MAGRA 已尝试桥接但失败。常见原因：URL 错误、需要鉴权、上游 404 / 5xx、本地命令缺失。修复后重启 magra code 重试。",
    ...
    marketplaceEnvHint: "下次启动 `magra code` 之前在 shell 里设好，桥接的服务器才能正常鉴权。",
    marketplaceRestartHint:
      "已写入 ~/.reasonix/config.json。重启 `magra code` 后服务器才会真正桥接（热重载在路线图上）。",
  },
  memory: {
    loading: "加载记忆…",
    files: "记忆 · {count} 个文件",
  ...
  hooks: {
    ...
    colScript: "脚本",
    noProject: "无活跃项目 — 在 magra code 中打开 /dashboard 以编辑项目钩子。",
    saveReload: "保存并重载",
    discard: "放弃更改",
  ...
  skills: {
    ...
    builtinDesc:
      "内置技能随 MAGRA 一起发布；模型会自动识别。如需自定义，请创建同名的项目或全局技能。",
  ...
  semantic: {
    ...
    installOllamaDesc: "MAGRA 不会为您运行包管理器。请先安装 Ollama，然后返回：",
    ...
    remoteProviderDesc:
      "在这里配置 OpenAI-Compatible embeddings 的完整 URL。MAGRA 会严格使用你提供的 URL 发起请求。",
    ollama: "Ollama",
    binary: "二进制",
    found: "已找到",
    missing: "缺失",
  ...
  statusbar: {
    ...
    exchangeRatePending: "汇率加载中",
    exchangeRateUpdated: "更新于 {time}",
    jobs: "任务",
    jobsTip: "后台任务",
    offline: "离线",
    online: "在线",
    rtk: "RTK",
    rtkPending: "RTK 节省统计加载中",
    rtkSessionSaved: "本会话 RTK 节省 {tokens} tokens",
    rtkTotalSaved: "累计节省 {tokens}",
    rtkUnavailable: "RTK 节省统计不可用",
    switchCurrency: "切换货币",
    switchCurrencyTo: "切换到 {currency}",
    switchTheme: "切换主题",
